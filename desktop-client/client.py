...
class SyncEngine:
    ...
    def pull_incoming(self):
        ...
        for notif in notifications:
            # Notification data may be a JSON string from the server
            ...
            if notif["type"] == "item_sold":
                incoming_data["ah_purchases"].append({
                    ...
                    "buyer": ndata.get("buyer", ""),
                    "seller": self.player_name,
                    "state": "awaiting_cod",
                    "itemName": ndata.get("item_name", ""),
                    "price": ndata.get("price", 0),
                    "quantity": ndata.get("quantity", 1),
                })
            ...
            for n in notifs:
                ...
                if ntype == "item_sold":
                    ...
                    self._desktop_notify(
                        "Item Sold!",
                        f"{item_name} purchased by {buyer} for {price:,}g\n"
                        f"Send them a COD mail to complete the sale!"
                    )
