            ...
            if isinstance(actions, dict):
                for key, action in sorted(actions.items(), key=lambda x: str(x[0])):
                    if isinstance(action, dict) and action.get("action"):
                        # Skip actions we've already pushed (track by a hash of the action)
                        action_id = f"{action.get('action')}_{action.get('data', {}).get('id', key)}"
                ...
                for i, action in enumerate(actions):
                    if isinstance(action, dict) and action.get("action"):
                        action_id = f"{action.get('action')}_{action.get('data', {}).get('id', i)}"
                        if action_id in self._pushed_action_ids:
                            continue
                        action_list.append(action)
                        self._pending_action_ids.append(action_id)
