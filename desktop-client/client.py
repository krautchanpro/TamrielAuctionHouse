...
class SavedVarsManager:
    ...
    def write_metadata(self, last_sync: int, sync_count: int = 0):
        """Update the metadata section in SavedVariables so the addon knows we're connected."""
        if not self.sv_file.exists():
            return

        content = self.sv_file.read_text(encoding="utf-8", errors="replace")

        # Update last_sync value
        sync_pattern = r'(\["last_sync"\]\s*=\s*)\d+'
        if re.search(sync_pattern, content):
            content = re.sub(sync_pattern, f'["last_sync"] = {last_sync}', content)

        # Update client_version
        version_pattern = r'(\["client_version"\]\s*=\s*)"[^"]*"'
        if re.search(version_pattern, content):
            content = re.sub(version_pattern, f'["client_version"] = "1.0.0"', content)

        # Update sync_count
        count_pattern = r'(\["sync_count"\]\s*=\s*)\d+'
        if re.search(count_pattern, content):
            content = re.sub(count_pattern, f'["sync_count"] = {sync_count}', content)
